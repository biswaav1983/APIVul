...
JWT_PASS = environ.get("JWT_PASS", str(uuid4()))
...
def login():
    ...
    if "email" in data and "password" in data:
        try:
            ...
            if bcrypt.checkpw(
                data.get("password").encode(), ref_user.password.encode()
            ):
                token = jwt.encode(
                    {"user": data.get("email")}, JWT_PASS, algorithm="HS256"
                )
                return jsonify(
                    {
                        "success": True,
    ...
    if email:
        ...
        if validate_token:
            ...
            ref_cards = (
                PaymentCard.select()
                .join(User)
                .where(User.email == validate_token.get("user"))
            )
...
def get_card(card_id):
    if card_id:
        ...
        if validate_token:
            try:
                ref_card = PaymentCard.get(PaymentCard.id == int(card_id))
                return jsonify(
                    {
                        "success": True,
                        "error": False,
